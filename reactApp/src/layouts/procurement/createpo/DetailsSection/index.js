...
import DetailsForm from "../../../../examples/Forms/FormMenu/DetailsForm";


const MemoizedDetailsForm = React.memo(DetailsForm);

export default function DetailsSection({ detailsForms, handleDetailsFormData, dropDetailsForm, error }) {
  return (
    <>
        ...
        <MDBox component="ul" display="flex" flexDirection="column">
          <MemoizedDetailsForm
           onDataUpdate={(description, section, machine, quantity, unit, price, total) => 
            handleDetailsFormData(index, description, section, machine, quantity, unit, price, total)}
            index={index} 
            error={error}
          />
        </MDBox>
