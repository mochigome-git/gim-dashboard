...
export const dataCSV = async (date_at, seq, tableName, folderName, fivemin) => {
    try {
        ...
        const newHeader = replaceHeaderColumnNames(existingHeader, columnReplacements);

        // Create a new array with the updated header followed by the data rows
        const updatedData = [newHeader, ...selectedData.map(row => Object.values(row))];
...
export const multipleDataCSV = async (dates, seqArray, tableName, folderName, fivemin) => {
    try {
        ...
        // Create a new array with the updated header followed by the data rows
        const updatedData = [newHeader, ...allData.map(row => Object.values(row))];
