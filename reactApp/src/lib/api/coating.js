...
export const fetchNk2Details = async (setState, date, seq) => {
  ...
  try {
    ...
    //});
    const { data: data1, error: error1 } = await supabase.from("nk2_log_data_storage")
      .select("d608, d609, d610, d611, d612, d613, d614,d800, d802, d804, d806, d808, d810, d812, d814, d816,d534, d536, d538, d540, d542, d544, d546,created_at, i_h_seq")
      .gte("created_at", `${date}T00:00:00.000Z`)
      .lt("created_at", `${nextDay(date)}T00:00:00.000Z`)
      .eq("i_h_seq", seq)
      .order("created_at");

    const { data: data2, error: error2 } = await supabase.rpc("get_nk2_2u_fibre_sensor", {
      seq: seq, date_at: date,
    });
    ...
    const { data: data6, error: error6 } = await supabase.from("nk2_log_data_storage")
    ...
    const { data: data7, error: error7 } = await supabase.from("nk2_log_data_storage")
...
export const fetchNk3Details = async (setState, date, seq) => {
  ...
  try {
    ...
    //  seq: seq, date_at: date,
    //});
    const { data: data1, error: error1 } = await supabase.from("nk3_log_data_storage")
      .select("d608, d609, d610, d611, d612, d613, d614,d800, d802, d804, d806, d808, d810, d812, d814, d816,d534, d536, d538, d540, d542, d544, d546,created_at, i_h_seq")
      .gte("created_at", `${date}T00:00:00.000Z`)
      .lt("created_at", `${nextDay(date)}T00:00:00.000Z`)
      .eq("i_h_seq", seq)
    ...
    const { data: data2, error: error2 } = await supabase.rpc("get_nk3_2u_fibre_sensor", {
      seq: seq, date_at: date,
    });

    const { data: data3, error: error3 } = await supabase.from("nk3_4u_fibre_sensor")
      .select("*")
      .gte("created_at", `${date}T00:00:00.000Z`)
      .lt("created_at", `${nextDay(date)}T00:00:00.000Z`)
      .eq("i_h_seq", seq)
    ...
    const { data: data4, error: error4 } = await supabase.from("nk3_main_pressure_sensor")
      .select("*")
      .gte("created_at", `${date}T00:00:00.000Z`)
      .lt("created_at", `${nextDay(date)}T00:00:00.000Z`)
      .eq("i_h_seq", seq)
    ...
    const { data: data5, error: error5 } = await supabase.from("nk3_log_data_storage")
    ...
    const { data: data6, error: error6 } = await supabase.from("nk3_log_data_storage")
...
export const fetchTableData = async (table, stateName, setState, hoursOffset = 0) => {
  try {
